...
import { Codes as codes } from './codes.js';
...
class PokeDex {
	...
	isRegional(id, form="") {
		let temp = JSON.parse(JSON.stringify(this.entries[id]));
		if (temp.hasOwnProperty('forms') && form != "") { temp = Object.assign({}, temp, temp.forms[form]); }

		if (temp.hasOwnProperty('region')) {
			let mon = {};
			mon.name = temp.name;
			mon.form = form;
			mon.region = temp.region;
			return mon;
		}
		return false;
	}
