...
export class Autocomplete {
	constructor(field, options) {
		...
		field.addEventListener('keydown', (e) => {
			...
			if (e.keyCode === 40) {
				...
				this.dropdown._menu.children[this.dropdown._menu.dataset.focus]?.classList.add('active');
			...
			if (e.keyCode === 13) {
				let target = this.dropdown._menu.children[this.dropdown._menu.dataset.focus];

				let dataLabel = target.getAttribute('data-label');
	...
	createItem(lookup, item) {
		...
		return ce(`<li><a href="#/dex/${item.value}" class="dropdown-item" data-label="${item.label}" data-value="${ item.value.toString().padStart(3,'0') }" value="${item.value}">${label}</a></li>`);
	}
