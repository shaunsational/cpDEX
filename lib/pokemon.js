...
export class Pokemon {
	...
	sprite(id, form, normal, shiny=false) {
		...
		let sprite = (form == "") ? id : `${id}-${form.toLowerCase()}`;

		let img = $('#pokemon #sprite img');
		img.src = `./sprites/${sprite}.png`;
		img.setAttribute('data-sprite', `./sprites/${sprite}.png`);
		img.setAttribute('data-shiny', `./sprites/shiny/${sprite}.png`);
	}
	...
	updateMon(mon, currentForm="") {
		...
		if (mon.hasOwnProperty('forms') && mon.forms.size > 1) {
		...
		if (mon.hasOwnProperty('region')) {
			...
			if (evo !== false) {
				this.pokeNote('globe', `This pokémon evolves from ${evo.name} (${evo.form}) which can be found in <span id="region">${evo.region}</span>, and/or possibly during special events.`, 'regional');
			}
		}

		$('#pokemon #sprite').classList.remove('shiny');
		this.sprite(mon.id, currentForm, mon.default);
