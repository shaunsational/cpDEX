...
export class Pokemon {
	...
	updateMon(mon, currentForm="") {
		...
		$('#pokemon .banner').style.background = 'linear-gradient(to bottom, rgba(var(--pokergb-'+ mon.type[0] +'),.25), transparent)';
		...
		if (mon.hasOwnProperty('released') && !mon.released) {
			this.pokeNote('slash', `${mon.name} has not been released in Pokémon GO yet.`);
		}
	...
	cleanSlate(el) {
		...
		*/
		...
		/*
		'<div id="pokemonAvailability" class="pokenote">'+
			'<span data-feather="slash"></span>'+
			'<p></p>'+
		'</div>'+

		'<div id="pokemonNote" class="pokenote">'+
			'<span data-feather="alert-triangle"></span>'+
			'<p></p>'+
		'</div>'+

		'<div id="pokemonRegional" class="pokenote">'+
			'<span data-feather="globe"></span>'+
			'<p>This regional pokémon can be found in <span id="region"></span>, and/or possibly during special events.</p>'+
		'</div>'+
		*/
