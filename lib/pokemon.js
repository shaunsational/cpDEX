import { fetchJSON, isDay, loadMsg, Dex } from './modules.js';
...
export class Pokemon {
	...
	updateMon(mon) {
		...
		$('#megas').innerHTML = '';
		// TODO REDO THIS TO ONLY SHOW EACH POKEMON ONCE EVEN IF IT HAS MULTIPLE TYPES
					let buddies = {};
					mon.type.forEach(t => {
		...
		for (let i = 0; i < mon.cp_range.length; i++) {
			//table.innerHTML += `<tr><td>${mon.cp_range[i].level}</td><td>${mon.cp_range[i].min}</td><td>${mon.cp_range[i].max}</td></tr>`;

			if (i+1 == 15) { // FIELD RESEARCH
				$('#apb .research').innerHTML = '<i class="activity"></i><span class="cp">'+ mon.cp_range[i].max +'CP</span>';
			} 
			if (i+1 == 20) { // HATCHED / RAID
				$('#apb .hatched').innerHTML = '<i class="activity"></i><span class="cp">'+ mon.cp_range[i].max +'CP</span>';
				$('#apb .raid').innerHTML = '<i class="activity"></i><span class="cp">'+ mon.cp_range[i].max +'CP</span>';
			}
			if (i+1 == 25) { // RAID WEATHER
				$('#apb .raid-w').innerHTML = '<i class="activity"></i><span class="cp">'+ mon.cp_range[i].max +'CP</span><span class="boost">' + monTypes.boost +'</span>';
			}
			if (i+1 == 30) { // WILD
				$('#apb .wild').innerHTML = '<i class="activity"></i><span class="cp">'+ mon.cp_range[i].max +'CP</span>';
			}
			if (i+1 == 35) { // WILD WEATHER
				$('#apb .wild-w').innerHTML = '<i class="activity"></i><span class="cp">'+ mon.cp_range[i].max +'CP</span><span class="boost">' + monTypes.boost +'</span>';
			}
			...
			//levels.push(`<div data-i="${i+1}" data-c="${(i) % 5}" data-r="${r}" style="order: ${(i) % 5 * 5 + r} "><span class="level">${mon.cp_range[i].level}</span><span class="min">${mon.cp_range[i].min}</span><span class="max">${mon.cp_range[i].max}</span></div>`);
	...
	cleanSlate(el) {
		...
		slate.innerHTML = '<h1 id="mon-name"></h1>'+
		'<div id="apb"><div class="wild-w"></div><div class="wild"></div><div class="raid-w"></div><div class="raid"></div><div class="research"></div><div class="hatched"></div></div>'+
		/*
		...
		*/
		...
		'<div class="table-responsive grid-table">'+
			'<aside>'+
				'<div><b>Level</b><b>Min</b><b>Max</b></div>'+
				'<div><b>Level</b><b>Min</b><b>Max</b></div>'+
				'<div class="x2-hide"><b>Level</b><b>Min</b><b>Max</b></div>'+
				'<div class="x2-hide"><b>Level</b><b>Min</b><b>Max</b></div>'+
				'<div class="x2-hide"><b>Level</b><b>Min</b><b>Max</b></div>'+
			'</aside>'+
